      ...
      <div class="teks-berjalan">
       ...
       <nav>
        <ul>
          ...
          <li  class="active" id="hover"><a href="dataharian.php">DATA BULANAN</a></li>
    ...
    <div class="box">
      ...
      <div id="cards" class="cards" align="center">
        ...
        <table id="wnntable" style="border-radius: 12px;">
          ...
          // $tgl_hari_ini=date("Y-m");
          $bln = date("m");
          // $tampil = query("SELECT * FROM datasensor WHERE MONTH(waktu) = '".$bln."' ");
          $data = mysqli_fetch_array($tampil); 
           ?>

          <?php $jumlahdataperhalaman = 5;
          // $jumlahdata = count(query("SELECT * FROM datasensor WHERE waktu='$tgl_hari_ini' ORDER BY id DESC"));
          $jumlahdata = count(query("select concat(month(tgl_hari_ini)) as tahun_bulan, count(*) as jumlah,SUM(suhu) as temp from datasensor group by month(tgl_hari_ini) ORDER BY id DESC"));
          // echo $jumlahdata;
          // die();
          ...
          $show = query("select concat(year(tgl_hari_ini),' / ',month(tgl_hari_ini)) as tahun_bulan, count(*) as jumlah,AVG(suhu) as temp,AVG(kelembaban) as humi from datasensor group by year(tgl_hari_ini),month(tgl_hari_ini) ORDER BY id DESC LIMIT $awaldata, $jumlahdataperhalaman");
          $i= 1 + $awaldata;
          $jumlahlink = 4;
