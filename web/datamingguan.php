      ...
      <div class="teks-berjalan">
       ...
       <nav>
        <ul>
          ...
          <li  class="active" id="hover"><a href="dataharian.php">DATA M</a></li>
      ...
      <div id="cards" class="cards" align="center">
        <table id="wnntable" style="border-radius: 12px;">
          <tr>
            ...
            Rata-rata Suhu Dan Kelembaban Mingguan
            </th>
          </tr>
          <tr >
            <th>No</th>
          ...
          ?>
          <?php $jumlahdataperhalaman = 2;
          $jumlahdata = count(query("select concat(year(tgl_hari_ini),week(tgl_hari_ini)) as tahun_minggu from datasensor group by year(tgl_hari_ini),week(tgl_hari_ini)"));
          // $jumlahdata = count(query("SELECT * FROM datasensor WHERE waktu='$week' ORDER BY id DESC"));          
          // echo $jumlahdata;
          // die();
          ...
          $show = query("select concat(year(tgl_hari_ini),' / ',week(tgl_hari_ini)) as tahun_minggu, count(*) as jumlah,AVG(suhu) as temp,AVG(kelembaban) as humi from datasensor group by year(tgl_hari_ini),week(tgl_hari_ini) ORDER BY id DESC LIMIT $awaldata, $jumlahdataperhalaman");
          $i= 1 + $awaldata;
          $jumlahlink = 4;
